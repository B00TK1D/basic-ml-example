    ...
    def train(model, data, loss_fn, optimizer, num_epochs):
        for epoch in range(num_epochs):
            for input_sequence, target in data:
                input_sequence = torch.Tensor(input_sequence).view(
                    len(input_sequence), 1, -1)
...
data = [(list(range(10)), list(range(1, 11))),
        (list(range(10, 20)), list(range(11, 21)))]
print(data)
loss_fn = nn.MSELoss()
optimizer = torch.optim.Adam(model.parameters())

# Train the model
NextNumberPredictor.train(model, data, loss_fn, optimizer, 100)
